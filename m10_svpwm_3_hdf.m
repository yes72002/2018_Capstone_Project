clear%7/7,7/13,7/17,7/18,7/19,7/20誤差HDF
close all
clc
time = 0.02;
fsw = 36000;
...
for i=1:1:fsw*time
    if mod(sector(i),2)==1%返回1,sector就是奇數；返回0,sector就是偶數
        VerrorD1(i)=(2/3)*sin(angle(i)-(pi/3)*(sector(i)-1));
        ...
        VerrorD1(i)=(2/3)*sin((pi/3)*sector(i)-angle(i));
    ...
    Qz(i)=VerrorQz(i)*Tz(i);
    D1(i)=VerrorD1(i)*Tless(i);
    D2(i)=VerrorD2(i)*Tmore(i);
    Dz(i)=VerrorDz(i)*Tz(i);
end
