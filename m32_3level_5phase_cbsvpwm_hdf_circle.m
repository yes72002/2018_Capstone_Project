...
for j=1:r
    Dutym = Dutyz/2;
    X0_p1(j,:) = (vdq0_p1x(j,:)-sindq_p1x(j,:)).*Dutym(j,:);%v0在dq平面1上x軸
    ...
    X5_p1(j,:) = (vdq5_p1x(j,:)-sindq_p1x(j,:)).*Dutym(j,:);%v5在dq平面1上x軸
    Y0_p1(j,:) = (vdq0_p1y(j,:)-sindq_p1y(j,:)).*Dutym(j,:);%v0在dq平面1上y軸
    ...
    Y5_p1(j,:) = (vdq5_p1y(j,:)-sindq_p1y(j,:)).*Dutym(j,:);%v5在dq平面1上y軸
    X0_p2(j,:) = (vdq0_p2x(j,:)-sindq_p2x(j,:)).*Dutym(j,:);%v0在dq平面2上x軸
    ...
    X5_p2(j,:) = (vdq5_p2x(j,:)-sindq_p2x(j,:)).*Dutym(j,:);%v5在dq平面2上x軸
    Y0_p2(j,:) = (vdq0_p2y(j,:)-sindq_p2y(j,:)).*Dutym(j,:);%v0在dq平面2上y軸
    ...
    Y5_p2(j,:) = (vdq5_p2y(j,:)-sindq_p2y(j,:)).*Dutym(j,:);%v5在dq平面2上y軸
end
Xz_p1 = X0_p1 + X5_p1;
Xz_p2 = X0_p2 + X5_p2;
Yz_p1 = Y0_p1 + Y5_p1;
...
for i=1:fsw*time%Plane1
    ...
    HDFX_p1(j,i)=((P1)^2+(P1)*(P2)+(P2)^2)*Tz(j,i)/2 ...
    ...
    HDFY_p1(j,i)=((R1)^2+(R1)*(R2)+(R2)^2)*Tz(j,i)/2 ...
...
for i=1:fsw*time%Plane2
    ...
    HDFX_p2(j,i)=((P1)^2+(P1)*(P2)+(P2)^2)*Tz(j,i)/2 ...
              +((P2)^2+(P2)*(P3)+(P3)^2)*T1(j,i)...
              +((P3)^2+(P3)*(P4)+(P4)^2)*T2(j,i)...
              +((P4)^2+(P4)*(P5)+(P5)^2)*T3(j,i)...
              +((P5)^2+(P5)*(P6)+(P6)^2)*T4(j,i)...
    ...
    HDFY_p2(j,i)=((R1)^2+(R1)*(R2)+(R2)^2)*Tz(j,i)/2 ...
              +((R2)^2+(R2)*(R3)+(R3)^2)*T1(j,i)...
              +((R3)^2+(R3)*(R4)+(R4)^2)*T2(j,i)...
              +((R4)^2+(R4)*(R5)+(R5)^2)*T3(j,i)...
              +((R5)^2+(R5)*(R6)+(R6)^2)*T4(j,i)...
