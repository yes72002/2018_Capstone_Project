...
for i=1:fsw*time
    ...
    P3 = Xz(i)+X1(i);
    P4 = Xz(i)+X1(i)+X2(i);
    HDF012X1(i)=((P1)^2+(P1)*(P2)+(P2)^2)*Tz(i);
    HDF012X2(i)=((P2)^2+(P2)*(P3)+(P3)^2)*T1(i);
    HDF012X3(i)=((P3)^2+(P3)*(P4)+(P4)^2)*T2(i);
    R1 = 0;
    R2 = Yz(i);
    R3 = Yz(i)+Y1(i);
    R4 = Yz(i)+Y1(i)+Y2(i);
    HDF012Y1(i)=((R1)^2+(R1)*(R2)+(R2)^2)*Tz(i);
    HDF012Y2(i)=((R2)^2+(R2)*(R3)+(R3)^2)*T1(i);
    HDF012Y3(i)=((R3)^2+(R3)*(R4)+(R4)^2)*T2(i);
    %HDF721
    P1 = 0;
    P2 = Xz(i);
    P3 = Xz(i)+X2(i);
    P4 = Xz(i)+X2(i)+X1(i);
    HDF721X1(i)=((P1)^2+(P1)*(P2)+(P2)^2)*Tz(i);
    HDF721X2(i)=((P2)^2+(P2)*(P3)+(P3)^2)*T2(i);
    HDF721X3(i)=((P3)^2+(P3)*(P4)+(P4)^2)*T1(i);
    R1 = 0;
    R2 = Yz(i);
    R3 = Yz(i)+Y2(i);
    R4 = Yz(i)+Y2(i)+Y1(i);
    HDF721Y1(i)=((R1)^2+(R1)*(R2)+(R2)^2)*Tz(i);
    HDF721Y2(i)=((R2)^2+(R2)*(R3)+(R3)^2)*T2(i);
    HDF721Y3(i)=((R3)^2+(R3)*(R4)+(R4)^2)*T1(i);
end
...
HDF012X = HDF012X1+HDF012X2+HDF012X3;
HDF012Y = HDF012Y1+HDF012Y2+HDF012Y3;
HDF012 = HDF012X+HDF012Y;
HDF721X = HDF721X1+HDF721X2+HDF721X3;
HDF721Y = HDF721Y1+HDF721Y2+HDF721Y3;
HDF721 = HDF721X+HDF721Y;
XXX=X1+X2+Xz;
YYY=Y1+Y2+Yz;
...
plot(angle*180/pi,HDF012,'k'),hold on;
plot(angle*180/pi,HDF721,'m'),hold on;
% plot(angle*180/pi,minHDFvalue3,'m'),hold on;
legend('HDF','HDF0121','HDF7212','HDF012','HDF721');