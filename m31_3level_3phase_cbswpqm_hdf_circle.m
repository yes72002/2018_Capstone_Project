...
vrefy = sindqy;
for j=1:r
    X0(j,:) = (vdq0x(j,:)-vrefx(j,:)).*Dutyz(j,:)/2;  
    X1(j,:) = (vdq1x(j,:)-vrefx(j,:)).*Duty1(j,:);
    X2(j,:) = (vdq2x(j,:)-vrefx(j,:)).*Duty2(j,:);
    X3(j,:) = (vdq3x(j,:)-vrefx(j,:)).*Dutyz(j,:)/2;
    Y0(j,:) = (vdq0y(j,:)-vrefy(j,:)).*Dutyz(j,:)/2;
    Y1(j,:) = (vdq1y(j,:)-vrefy(j,:)).*Duty1(j,:);
    Y2(j,:) = (vdq2y(j,:)-vrefy(j,:)).*Duty2(j,:);
    Y3(j,:) = (vdq3y(j,:)-vrefy(j,:)).*Dutyz(j,:)/2;
end
Xz = X0 + X3;
Yz = Y0 + Y3;
T1 = Duty1;%無限跟有限精準度
